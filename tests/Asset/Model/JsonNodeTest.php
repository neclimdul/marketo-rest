<?php

/**
 * JsonNodeTest class file.
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Please update the test case below to test the model.
 */

declare(strict_types=1);

namespace NecLimDul\MarketoRest\Asset\Test\Model;

use Faker\Factory as FakerFactory;
use NecLimDul\MarketoRest\Asset\Model\ModelInterface;
use NecLimDul\MarketoRest\Asset\Model\JsonNode;
use Neclimdul\OpenapiPhpTesting\FakeValuesTrait;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

/**
 * JsonNodeTest
 *
 * The version of the OpenAPI document: 1.0
 * Contact: 
 * Generated by: https://openapi-generator.tech
 * OpenAPI Generator version: 7.21.0
 */
#[CoversClass(JsonNode::class)]
class JsonNodeTest extends TestCase
{
    use FakeValuesTrait;

    private JsonNode $sot;

    /**
     * @var string[]
     */
    private array $types = [
        'array' => 'bool',
        'big_decimal' => 'bool',
        'big_integer' => 'bool',
        'binary' => 'bool',
        'boolean' => 'bool',
        'container_node' => 'bool',
        'double' => 'bool',
        'float' => 'bool',
        'floating_point_number' => 'bool',
        'int' => 'bool',
        'integral_number' => 'bool',
        'long' => 'bool',
        'missing_node' => 'bool',
        'node_type' => 'string',
        'null' => 'bool',
        'number' => 'bool',
        'object' => 'bool',
        'pojo' => 'bool',
        'short' => 'bool',
        'textual' => 'bool',
        'value_node' => 'bool',
    ];

    /**
     * @var scalar[][]
     */
    private array $allowedValues = [
        'node_type' => [
            '_ARRAY' => 'ARRAY',
            'BINARY' => 'BINARY',
            'BOOLEAN' => 'BOOLEAN',
            'MISSING' => 'MISSING',
            'NULL' => 'NULL',
            'NUMBER' => 'NUMBER',
            'OBJECT' => 'OBJECT',
            'POJO' => 'POJO',
            'STRING' => 'STRING',
        ],
    ];

    public function setUp(): void
    {
        parent::setUp();
        $this->setFaker(FakerFactory::create());
        $this->sot = $this->getFakeValue(JsonNode::class, null);
    }

    /**
     * Test "JsonNode"
     *
     * @legacy-covers ::__construct
     */
    public function testJsonNode(): void
    {
        $this->assertInstanceOf(JsonNode::class, $this->sot);
    }

    /**
     * @legacy-covers ::openAPITypes
     */
    public function testOpenApiTypes(): void
    {
        $this->assertEquals($this->types, JsonNode::openAPITypes());
    }

    /**
     * @legacy-covers ::openAPIRequired
     */
    public function testOpenAPIRequired(): void
    {
        $required = $this->sot->openAPIRequired();
        $this->assertNotContains('array', $required);
        $this->assertNotContains('big_decimal', $required);
        $this->assertNotContains('big_integer', $required);
        $this->assertNotContains('binary', $required);
        $this->assertNotContains('boolean', $required);
        $this->assertNotContains('container_node', $required);
        $this->assertNotContains('double', $required);
        $this->assertNotContains('float', $required);
        $this->assertNotContains('floating_point_number', $required);
        $this->assertNotContains('int', $required);
        $this->assertNotContains('integral_number', $required);
        $this->assertNotContains('long', $required);
        $this->assertNotContains('missing_node', $required);
        $this->assertNotContains('node_type', $required);
        $this->assertNotContains('null', $required);
        $this->assertNotContains('number', $required);
        $this->assertNotContains('object', $required);
        $this->assertNotContains('pojo', $required);
        $this->assertNotContains('short', $required);
        $this->assertNotContains('textual', $required);
        $this->assertNotContains('value_node', $required);
    }

    /**
     * @legacy-covers ::openAPIFormats
     */
    public function testOpenAPIFormats(): void
    {
        $formats = $this->sot->openAPIFormats();
        $this->assertNotContains('array', $formats);
        $this->assertNotContains('big_decimal', $formats);
        $this->assertNotContains('big_integer', $formats);
        $this->assertNotContains('binary', $formats);
        $this->assertNotContains('boolean', $formats);
        $this->assertNotContains('container_node', $formats);
        $this->assertNotContains('double', $formats);
        $this->assertNotContains('float', $formats);
        $this->assertNotContains('floating_point_number', $formats);
        $this->assertNotContains('int', $formats);
        $this->assertNotContains('integral_number', $formats);
        $this->assertNotContains('long', $formats);
        $this->assertNotContains('missing_node', $formats);
        $this->assertNotContains('node_type', $formats);
        $this->assertNotContains('null', $formats);
        $this->assertNotContains('number', $formats);
        $this->assertNotContains('object', $formats);
        $this->assertNotContains('pojo', $formats);
        $this->assertNotContains('short', $formats);
        $this->assertNotContains('textual', $formats);
        $this->assertNotContains('value_node', $formats);
    }

    /**
     * @legacy-covers ::attributeMap
     */
    public function testAttributeMap(): void
    {
        $attributes = $this->sot->attributeMap();
        $this->assertEquals('array', $attributes['array']);
        $this->assertEquals('bigDecimal', $attributes['big_decimal']);
        $this->assertEquals('bigInteger', $attributes['big_integer']);
        $this->assertEquals('binary', $attributes['binary']);
        $this->assertEquals('boolean', $attributes['boolean']);
        $this->assertEquals('containerNode', $attributes['container_node']);
        $this->assertEquals('double', $attributes['double']);
        $this->assertEquals('float', $attributes['float']);
        $this->assertEquals('floatingPointNumber', $attributes['floating_point_number']);
        $this->assertEquals('int', $attributes['int']);
        $this->assertEquals('integralNumber', $attributes['integral_number']);
        $this->assertEquals('long', $attributes['long']);
        $this->assertEquals('missingNode', $attributes['missing_node']);
        $this->assertEquals('nodeType', $attributes['node_type']);
        $this->assertEquals('null', $attributes['null']);
        $this->assertEquals('number', $attributes['number']);
        $this->assertEquals('object', $attributes['object']);
        $this->assertEquals('pojo', $attributes['pojo']);
        $this->assertEquals('short', $attributes['short']);
        $this->assertEquals('textual', $attributes['textual']);
        $this->assertEquals('valueNode', $attributes['value_node']);
    }

    /**
     * @legacy-covers ::getters
     * @legacy-covers ::setters
     */
    public function testGettersSetters(): void
    {
        $getters = $this->sot->getters();
        $setters = $this->sot->setters();
        foreach (array_keys($this->types) as $field) {
            $this->assertTrue(isset($setters[$field]));
            $this->assertTrue(isset($getters[$field]));
        }
    }

    /**
     * @legacy-covers ::getModelName
     */
    public function testGetModelName(): void
    {
        $this->assertEquals('JsonNode', $this->sot->getModelName());
    }

    /**
     * @legacy-covers ::setAdditionalProperties
     * @legacy-covers ::setAdditionalProperty
     * @legacy-covers ::getAdditionalProperties
     */
    public function testAdditionalProperties(): void
    {
        do {
            $property = $this->faker->word();
        } while (isset($this->sot::attributeMap()[$property]));
        $value = 'foobar';
        $this->sot[$property] = $value;
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperty($property, $value);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperties([
            $property => $value,
        ]);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        $this->assertEquals(
            $value,
            $this->sot->jsonSerialize()->$property
        );
    }

    /**
     * @legacy-covers ::setAdditionalProperty
     */
    public function testAdditionalPropertyExistingProperty(): void
    {
        $this->expectException(\AssertionError::class);
        $this->sot->setAdditionalProperty(key($this->sot::attributeMap()), 'foobar');
    }

    /**
     * @legacy-covers ::jsonSerialize
     * @legacy-covers ::__toString
     */
    public function testJson(): void
    {
        // Some minimal tests that json generates well.
        $json = json_encode($this->sot);
        $this->assertIsString($json, 'Json encoded');
        $json = json_decode($json);
        $string = json_decode((string) $this->sot);
        $this->assertEquals(
            $json,
            $string
        );
        $this->assertInstanceOf(\stdClass::class, $json);
        $this->assertInstanceOf(\stdClass::class, $string);
    }

    /**
     * Test attribute "array"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getArray
     * @legacy-covers ::setArray
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyArray(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['array'],
            $this->allowedValues['array'] ?? null
        );
        $this->sot->setArray($v);
        $this->assertEquals($v, $this->sot->getArray());
        $this->sot->setArray(null);
        $this->assertNull($this->sot->getArray());
        $this->sot->setArray($v);

        $this->assertEquals($v, $this->sot['array']);
        $v = $this->getFakeValue(
            $this->types['array'],
            $this->allowedValues['array'] ?? null
        );
        $this->sot['array'] = $v;
        $this->assertTrue(isset($this->sot['array']));
        $this->assertEquals($v, $this->sot['array']);
        $this->assertEquals($v, $this->sot->getArray());
        unset($this->sot['array']);
        $this->assertFalse(isset($this->sot['array']));
        $this->sot['array'] = $v;
        $this->assertEquals($v, $this->sot['array']);
        $this->assertTrue(isset($this->sot['array']));
        $this->sot['array'] = null;
        $this->assertFalse(isset($this->sot['array']));
    }

    /**
     * Test attribute "big_decimal"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getBigDecimal
     * @legacy-covers ::setBigDecimal
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyBigDecimal(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['big_decimal'],
            $this->allowedValues['big_decimal'] ?? null
        );
        $this->sot->setBigDecimal($v);
        $this->assertEquals($v, $this->sot->getBigDecimal());
        $this->sot->setBigDecimal(null);
        $this->assertNull($this->sot->getBigDecimal());
        $this->sot->setBigDecimal($v);

        $this->assertEquals($v, $this->sot['big_decimal']);
        $v = $this->getFakeValue(
            $this->types['big_decimal'],
            $this->allowedValues['big_decimal'] ?? null
        );
        $this->sot['big_decimal'] = $v;
        $this->assertTrue(isset($this->sot['big_decimal']));
        $this->assertEquals($v, $this->sot['big_decimal']);
        $this->assertEquals($v, $this->sot->getBigDecimal());
        unset($this->sot['big_decimal']);
        $this->assertFalse(isset($this->sot['big_decimal']));
        $this->sot['big_decimal'] = $v;
        $this->assertEquals($v, $this->sot['big_decimal']);
        $this->assertTrue(isset($this->sot['big_decimal']));
        $this->sot['big_decimal'] = null;
        $this->assertFalse(isset($this->sot['big_decimal']));
    }

    /**
     * Test attribute "big_integer"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getBigInteger
     * @legacy-covers ::setBigInteger
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyBigInteger(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['big_integer'],
            $this->allowedValues['big_integer'] ?? null
        );
        $this->sot->setBigInteger($v);
        $this->assertEquals($v, $this->sot->getBigInteger());
        $this->sot->setBigInteger(null);
        $this->assertNull($this->sot->getBigInteger());
        $this->sot->setBigInteger($v);

        $this->assertEquals($v, $this->sot['big_integer']);
        $v = $this->getFakeValue(
            $this->types['big_integer'],
            $this->allowedValues['big_integer'] ?? null
        );
        $this->sot['big_integer'] = $v;
        $this->assertTrue(isset($this->sot['big_integer']));
        $this->assertEquals($v, $this->sot['big_integer']);
        $this->assertEquals($v, $this->sot->getBigInteger());
        unset($this->sot['big_integer']);
        $this->assertFalse(isset($this->sot['big_integer']));
        $this->sot['big_integer'] = $v;
        $this->assertEquals($v, $this->sot['big_integer']);
        $this->assertTrue(isset($this->sot['big_integer']));
        $this->sot['big_integer'] = null;
        $this->assertFalse(isset($this->sot['big_integer']));
    }

    /**
     * Test attribute "binary"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getBinary
     * @legacy-covers ::setBinary
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyBinary(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['binary'],
            $this->allowedValues['binary'] ?? null
        );
        $this->sot->setBinary($v);
        $this->assertEquals($v, $this->sot->getBinary());
        $this->sot->setBinary(null);
        $this->assertNull($this->sot->getBinary());
        $this->sot->setBinary($v);

        $this->assertEquals($v, $this->sot['binary']);
        $v = $this->getFakeValue(
            $this->types['binary'],
            $this->allowedValues['binary'] ?? null
        );
        $this->sot['binary'] = $v;
        $this->assertTrue(isset($this->sot['binary']));
        $this->assertEquals($v, $this->sot['binary']);
        $this->assertEquals($v, $this->sot->getBinary());
        unset($this->sot['binary']);
        $this->assertFalse(isset($this->sot['binary']));
        $this->sot['binary'] = $v;
        $this->assertEquals($v, $this->sot['binary']);
        $this->assertTrue(isset($this->sot['binary']));
        $this->sot['binary'] = null;
        $this->assertFalse(isset($this->sot['binary']));
    }

    /**
     * Test attribute "boolean"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getBoolean
     * @legacy-covers ::setBoolean
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyBoolean(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['boolean'],
            $this->allowedValues['boolean'] ?? null
        );
        $this->sot->setBoolean($v);
        $this->assertEquals($v, $this->sot->getBoolean());
        $this->sot->setBoolean(null);
        $this->assertNull($this->sot->getBoolean());
        $this->sot->setBoolean($v);

        $this->assertEquals($v, $this->sot['boolean']);
        $v = $this->getFakeValue(
            $this->types['boolean'],
            $this->allowedValues['boolean'] ?? null
        );
        $this->sot['boolean'] = $v;
        $this->assertTrue(isset($this->sot['boolean']));
        $this->assertEquals($v, $this->sot['boolean']);
        $this->assertEquals($v, $this->sot->getBoolean());
        unset($this->sot['boolean']);
        $this->assertFalse(isset($this->sot['boolean']));
        $this->sot['boolean'] = $v;
        $this->assertEquals($v, $this->sot['boolean']);
        $this->assertTrue(isset($this->sot['boolean']));
        $this->sot['boolean'] = null;
        $this->assertFalse(isset($this->sot['boolean']));
    }

    /**
     * Test attribute "container_node"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getContainerNode
     * @legacy-covers ::setContainerNode
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyContainerNode(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['container_node'],
            $this->allowedValues['container_node'] ?? null
        );
        $this->sot->setContainerNode($v);
        $this->assertEquals($v, $this->sot->getContainerNode());
        $this->sot->setContainerNode(null);
        $this->assertNull($this->sot->getContainerNode());
        $this->sot->setContainerNode($v);

        $this->assertEquals($v, $this->sot['container_node']);
        $v = $this->getFakeValue(
            $this->types['container_node'],
            $this->allowedValues['container_node'] ?? null
        );
        $this->sot['container_node'] = $v;
        $this->assertTrue(isset($this->sot['container_node']));
        $this->assertEquals($v, $this->sot['container_node']);
        $this->assertEquals($v, $this->sot->getContainerNode());
        unset($this->sot['container_node']);
        $this->assertFalse(isset($this->sot['container_node']));
        $this->sot['container_node'] = $v;
        $this->assertEquals($v, $this->sot['container_node']);
        $this->assertTrue(isset($this->sot['container_node']));
        $this->sot['container_node'] = null;
        $this->assertFalse(isset($this->sot['container_node']));
    }

    /**
     * Test attribute "double"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getDouble
     * @legacy-covers ::setDouble
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyDouble(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['double'],
            $this->allowedValues['double'] ?? null
        );
        $this->sot->setDouble($v);
        $this->assertEquals($v, $this->sot->getDouble());
        $this->sot->setDouble(null);
        $this->assertNull($this->sot->getDouble());
        $this->sot->setDouble($v);

        $this->assertEquals($v, $this->sot['double']);
        $v = $this->getFakeValue(
            $this->types['double'],
            $this->allowedValues['double'] ?? null
        );
        $this->sot['double'] = $v;
        $this->assertTrue(isset($this->sot['double']));
        $this->assertEquals($v, $this->sot['double']);
        $this->assertEquals($v, $this->sot->getDouble());
        unset($this->sot['double']);
        $this->assertFalse(isset($this->sot['double']));
        $this->sot['double'] = $v;
        $this->assertEquals($v, $this->sot['double']);
        $this->assertTrue(isset($this->sot['double']));
        $this->sot['double'] = null;
        $this->assertFalse(isset($this->sot['double']));
    }

    /**
     * Test attribute "float"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getFloat
     * @legacy-covers ::setFloat
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyFloat(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['float'],
            $this->allowedValues['float'] ?? null
        );
        $this->sot->setFloat($v);
        $this->assertEquals($v, $this->sot->getFloat());
        $this->sot->setFloat(null);
        $this->assertNull($this->sot->getFloat());
        $this->sot->setFloat($v);

        $this->assertEquals($v, $this->sot['float']);
        $v = $this->getFakeValue(
            $this->types['float'],
            $this->allowedValues['float'] ?? null
        );
        $this->sot['float'] = $v;
        $this->assertTrue(isset($this->sot['float']));
        $this->assertEquals($v, $this->sot['float']);
        $this->assertEquals($v, $this->sot->getFloat());
        unset($this->sot['float']);
        $this->assertFalse(isset($this->sot['float']));
        $this->sot['float'] = $v;
        $this->assertEquals($v, $this->sot['float']);
        $this->assertTrue(isset($this->sot['float']));
        $this->sot['float'] = null;
        $this->assertFalse(isset($this->sot['float']));
    }

    /**
     * Test attribute "floating_point_number"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getFloatingPointNumber
     * @legacy-covers ::setFloatingPointNumber
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyFloatingPointNumber(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['floating_point_number'],
            $this->allowedValues['floating_point_number'] ?? null
        );
        $this->sot->setFloatingPointNumber($v);
        $this->assertEquals($v, $this->sot->getFloatingPointNumber());
        $this->sot->setFloatingPointNumber(null);
        $this->assertNull($this->sot->getFloatingPointNumber());
        $this->sot->setFloatingPointNumber($v);

        $this->assertEquals($v, $this->sot['floating_point_number']);
        $v = $this->getFakeValue(
            $this->types['floating_point_number'],
            $this->allowedValues['floating_point_number'] ?? null
        );
        $this->sot['floating_point_number'] = $v;
        $this->assertTrue(isset($this->sot['floating_point_number']));
        $this->assertEquals($v, $this->sot['floating_point_number']);
        $this->assertEquals($v, $this->sot->getFloatingPointNumber());
        unset($this->sot['floating_point_number']);
        $this->assertFalse(isset($this->sot['floating_point_number']));
        $this->sot['floating_point_number'] = $v;
        $this->assertEquals($v, $this->sot['floating_point_number']);
        $this->assertTrue(isset($this->sot['floating_point_number']));
        $this->sot['floating_point_number'] = null;
        $this->assertFalse(isset($this->sot['floating_point_number']));
    }

    /**
     * Test attribute "int"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getInt
     * @legacy-covers ::setInt
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyInt(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['int'],
            $this->allowedValues['int'] ?? null
        );
        $this->sot->setInt($v);
        $this->assertEquals($v, $this->sot->getInt());
        $this->sot->setInt(null);
        $this->assertNull($this->sot->getInt());
        $this->sot->setInt($v);

        $this->assertEquals($v, $this->sot['int']);
        $v = $this->getFakeValue(
            $this->types['int'],
            $this->allowedValues['int'] ?? null
        );
        $this->sot['int'] = $v;
        $this->assertTrue(isset($this->sot['int']));
        $this->assertEquals($v, $this->sot['int']);
        $this->assertEquals($v, $this->sot->getInt());
        unset($this->sot['int']);
        $this->assertFalse(isset($this->sot['int']));
        $this->sot['int'] = $v;
        $this->assertEquals($v, $this->sot['int']);
        $this->assertTrue(isset($this->sot['int']));
        $this->sot['int'] = null;
        $this->assertFalse(isset($this->sot['int']));
    }

    /**
     * Test attribute "integral_number"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getIntegralNumber
     * @legacy-covers ::setIntegralNumber
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyIntegralNumber(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['integral_number'],
            $this->allowedValues['integral_number'] ?? null
        );
        $this->sot->setIntegralNumber($v);
        $this->assertEquals($v, $this->sot->getIntegralNumber());
        $this->sot->setIntegralNumber(null);
        $this->assertNull($this->sot->getIntegralNumber());
        $this->sot->setIntegralNumber($v);

        $this->assertEquals($v, $this->sot['integral_number']);
        $v = $this->getFakeValue(
            $this->types['integral_number'],
            $this->allowedValues['integral_number'] ?? null
        );
        $this->sot['integral_number'] = $v;
        $this->assertTrue(isset($this->sot['integral_number']));
        $this->assertEquals($v, $this->sot['integral_number']);
        $this->assertEquals($v, $this->sot->getIntegralNumber());
        unset($this->sot['integral_number']);
        $this->assertFalse(isset($this->sot['integral_number']));
        $this->sot['integral_number'] = $v;
        $this->assertEquals($v, $this->sot['integral_number']);
        $this->assertTrue(isset($this->sot['integral_number']));
        $this->sot['integral_number'] = null;
        $this->assertFalse(isset($this->sot['integral_number']));
    }

    /**
     * Test attribute "long"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getLong
     * @legacy-covers ::setLong
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyLong(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['long'],
            $this->allowedValues['long'] ?? null
        );
        $this->sot->setLong($v);
        $this->assertEquals($v, $this->sot->getLong());
        $this->sot->setLong(null);
        $this->assertNull($this->sot->getLong());
        $this->sot->setLong($v);

        $this->assertEquals($v, $this->sot['long']);
        $v = $this->getFakeValue(
            $this->types['long'],
            $this->allowedValues['long'] ?? null
        );
        $this->sot['long'] = $v;
        $this->assertTrue(isset($this->sot['long']));
        $this->assertEquals($v, $this->sot['long']);
        $this->assertEquals($v, $this->sot->getLong());
        unset($this->sot['long']);
        $this->assertFalse(isset($this->sot['long']));
        $this->sot['long'] = $v;
        $this->assertEquals($v, $this->sot['long']);
        $this->assertTrue(isset($this->sot['long']));
        $this->sot['long'] = null;
        $this->assertFalse(isset($this->sot['long']));
    }

    /**
     * Test attribute "missing_node"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getMissingNode
     * @legacy-covers ::setMissingNode
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyMissingNode(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['missing_node'],
            $this->allowedValues['missing_node'] ?? null
        );
        $this->sot->setMissingNode($v);
        $this->assertEquals($v, $this->sot->getMissingNode());
        $this->sot->setMissingNode(null);
        $this->assertNull($this->sot->getMissingNode());
        $this->sot->setMissingNode($v);

        $this->assertEquals($v, $this->sot['missing_node']);
        $v = $this->getFakeValue(
            $this->types['missing_node'],
            $this->allowedValues['missing_node'] ?? null
        );
        $this->sot['missing_node'] = $v;
        $this->assertTrue(isset($this->sot['missing_node']));
        $this->assertEquals($v, $this->sot['missing_node']);
        $this->assertEquals($v, $this->sot->getMissingNode());
        unset($this->sot['missing_node']);
        $this->assertFalse(isset($this->sot['missing_node']));
        $this->sot['missing_node'] = $v;
        $this->assertEquals($v, $this->sot['missing_node']);
        $this->assertTrue(isset($this->sot['missing_node']));
        $this->sot['missing_node'] = null;
        $this->assertFalse(isset($this->sot['missing_node']));
    }

    /**
     * Test attribute "node_type"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getNodeType
     * @legacy-covers ::setNodeType
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyNodeType(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['node_type'],
            $this->allowedValues['node_type'] ?? null
        );
        $this->sot->setNodeType($v);
        $this->assertEquals($v, $this->sot->getNodeType());
        $this->sot->setNodeType(null);
        $this->assertNull($this->sot->getNodeType());
        $this->sot->setNodeType($v);

        $this->assertEquals($v, $this->sot['node_type']);
        $v = $this->getFakeValue(
            $this->types['node_type'],
            $this->allowedValues['node_type'] ?? null
        );
        $this->sot['node_type'] = $v;
        $this->assertTrue(isset($this->sot['node_type']));
        $this->assertEquals($v, $this->sot['node_type']);
        $this->assertEquals($v, $this->sot->getNodeType());
        unset($this->sot['node_type']);
        $this->assertFalse(isset($this->sot['node_type']));
        $this->sot['node_type'] = $v;
        $this->assertEquals($v, $this->sot['node_type']);
        $this->assertTrue(isset($this->sot['node_type']));
        $this->sot['node_type'] = null;
        $this->assertFalse(isset($this->sot['node_type']));
    }

    /**
     * Trivial allowed values test.
     *
     * @legacy-covers ::getNodeTypeAllowableValues()
     */
    public function testNodeTypeAllowableValues(): void
    {
        // Trivial allowed values test.
        $this->assertEquals(array_values($this->allowedValues['node_type']), $this->sot->getNodeTypeAllowableValues());
        foreach ($this->allowedValues['node_type'] as $v) {
            $this->sot->setNodeType($v);
            $this->assertEquals($v, $this->sot->getNodeType());
        }
    }

    /**
     * @legacy-covers ::setNodeType
     */
    public function testNodeTypeNotAllowableValues(): void
    {
        $this->markTestIncomplete('Broken my property overloading and needs additional work');
        // Find a value that's not allowed.
        do {
            $v = $this->getFakeValue($this->types['node_type'], null);
        } while (!isset($v) || in_array($v, $this->allowedValues['node_type'], true));
        $this->expectException(\InvalidArgumentException::class);
        $this->sot->setNodeType($v);
        throw new \Exception(var_export($v, true) . " should have triggered an error...");
    }

    /**
     * Test attribute "null"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getNull
     * @legacy-covers ::setNull
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyNull(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['null'],
            $this->allowedValues['null'] ?? null
        );
        $this->sot->setNull($v);
        $this->assertEquals($v, $this->sot->getNull());
        $this->sot->setNull(null);
        $this->assertNull($this->sot->getNull());
        $this->sot->setNull($v);

        $this->assertEquals($v, $this->sot['null']);
        $v = $this->getFakeValue(
            $this->types['null'],
            $this->allowedValues['null'] ?? null
        );
        $this->sot['null'] = $v;
        $this->assertTrue(isset($this->sot['null']));
        $this->assertEquals($v, $this->sot['null']);
        $this->assertEquals($v, $this->sot->getNull());
        unset($this->sot['null']);
        $this->assertFalse(isset($this->sot['null']));
        $this->sot['null'] = $v;
        $this->assertEquals($v, $this->sot['null']);
        $this->assertTrue(isset($this->sot['null']));
        $this->sot['null'] = null;
        $this->assertFalse(isset($this->sot['null']));
    }

    /**
     * Test attribute "number"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getNumber
     * @legacy-covers ::setNumber
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyNumber(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['number'],
            $this->allowedValues['number'] ?? null
        );
        $this->sot->setNumber($v);
        $this->assertEquals($v, $this->sot->getNumber());
        $this->sot->setNumber(null);
        $this->assertNull($this->sot->getNumber());
        $this->sot->setNumber($v);

        $this->assertEquals($v, $this->sot['number']);
        $v = $this->getFakeValue(
            $this->types['number'],
            $this->allowedValues['number'] ?? null
        );
        $this->sot['number'] = $v;
        $this->assertTrue(isset($this->sot['number']));
        $this->assertEquals($v, $this->sot['number']);
        $this->assertEquals($v, $this->sot->getNumber());
        unset($this->sot['number']);
        $this->assertFalse(isset($this->sot['number']));
        $this->sot['number'] = $v;
        $this->assertEquals($v, $this->sot['number']);
        $this->assertTrue(isset($this->sot['number']));
        $this->sot['number'] = null;
        $this->assertFalse(isset($this->sot['number']));
    }

    /**
     * Test attribute "object"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getObject
     * @legacy-covers ::setObject
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyObject(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['object'],
            $this->allowedValues['object'] ?? null
        );
        $this->sot->setObject($v);
        $this->assertEquals($v, $this->sot->getObject());
        $this->sot->setObject(null);
        $this->assertNull($this->sot->getObject());
        $this->sot->setObject($v);

        $this->assertEquals($v, $this->sot['object']);
        $v = $this->getFakeValue(
            $this->types['object'],
            $this->allowedValues['object'] ?? null
        );
        $this->sot['object'] = $v;
        $this->assertTrue(isset($this->sot['object']));
        $this->assertEquals($v, $this->sot['object']);
        $this->assertEquals($v, $this->sot->getObject());
        unset($this->sot['object']);
        $this->assertFalse(isset($this->sot['object']));
        $this->sot['object'] = $v;
        $this->assertEquals($v, $this->sot['object']);
        $this->assertTrue(isset($this->sot['object']));
        $this->sot['object'] = null;
        $this->assertFalse(isset($this->sot['object']));
    }

    /**
     * Test attribute "pojo"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getPojo
     * @legacy-covers ::setPojo
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyPojo(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['pojo'],
            $this->allowedValues['pojo'] ?? null
        );
        $this->sot->setPojo($v);
        $this->assertEquals($v, $this->sot->getPojo());
        $this->sot->setPojo(null);
        $this->assertNull($this->sot->getPojo());
        $this->sot->setPojo($v);

        $this->assertEquals($v, $this->sot['pojo']);
        $v = $this->getFakeValue(
            $this->types['pojo'],
            $this->allowedValues['pojo'] ?? null
        );
        $this->sot['pojo'] = $v;
        $this->assertTrue(isset($this->sot['pojo']));
        $this->assertEquals($v, $this->sot['pojo']);
        $this->assertEquals($v, $this->sot->getPojo());
        unset($this->sot['pojo']);
        $this->assertFalse(isset($this->sot['pojo']));
        $this->sot['pojo'] = $v;
        $this->assertEquals($v, $this->sot['pojo']);
        $this->assertTrue(isset($this->sot['pojo']));
        $this->sot['pojo'] = null;
        $this->assertFalse(isset($this->sot['pojo']));
    }

    /**
     * Test attribute "short"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getShort
     * @legacy-covers ::setShort
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyShort(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['short'],
            $this->allowedValues['short'] ?? null
        );
        $this->sot->setShort($v);
        $this->assertEquals($v, $this->sot->getShort());
        $this->sot->setShort(null);
        $this->assertNull($this->sot->getShort());
        $this->sot->setShort($v);

        $this->assertEquals($v, $this->sot['short']);
        $v = $this->getFakeValue(
            $this->types['short'],
            $this->allowedValues['short'] ?? null
        );
        $this->sot['short'] = $v;
        $this->assertTrue(isset($this->sot['short']));
        $this->assertEquals($v, $this->sot['short']);
        $this->assertEquals($v, $this->sot->getShort());
        unset($this->sot['short']);
        $this->assertFalse(isset($this->sot['short']));
        $this->sot['short'] = $v;
        $this->assertEquals($v, $this->sot['short']);
        $this->assertTrue(isset($this->sot['short']));
        $this->sot['short'] = null;
        $this->assertFalse(isset($this->sot['short']));
    }

    /**
     * Test attribute "textual"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getTextual
     * @legacy-covers ::setTextual
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyTextual(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['textual'],
            $this->allowedValues['textual'] ?? null
        );
        $this->sot->setTextual($v);
        $this->assertEquals($v, $this->sot->getTextual());
        $this->sot->setTextual(null);
        $this->assertNull($this->sot->getTextual());
        $this->sot->setTextual($v);

        $this->assertEquals($v, $this->sot['textual']);
        $v = $this->getFakeValue(
            $this->types['textual'],
            $this->allowedValues['textual'] ?? null
        );
        $this->sot['textual'] = $v;
        $this->assertTrue(isset($this->sot['textual']));
        $this->assertEquals($v, $this->sot['textual']);
        $this->assertEquals($v, $this->sot->getTextual());
        unset($this->sot['textual']);
        $this->assertFalse(isset($this->sot['textual']));
        $this->sot['textual'] = $v;
        $this->assertEquals($v, $this->sot['textual']);
        $this->assertTrue(isset($this->sot['textual']));
        $this->sot['textual'] = null;
        $this->assertFalse(isset($this->sot['textual']));
    }

    /**
     * Test attribute "value_node"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getValueNode
     * @legacy-covers ::setValueNode
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyValueNode(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['value_node'],
            $this->allowedValues['value_node'] ?? null
        );
        $this->sot->setValueNode($v);
        $this->assertEquals($v, $this->sot->getValueNode());
        $this->sot->setValueNode(null);
        $this->assertNull($this->sot->getValueNode());
        $this->sot->setValueNode($v);

        $this->assertEquals($v, $this->sot['value_node']);
        $v = $this->getFakeValue(
            $this->types['value_node'],
            $this->allowedValues['value_node'] ?? null
        );
        $this->sot['value_node'] = $v;
        $this->assertTrue(isset($this->sot['value_node']));
        $this->assertEquals($v, $this->sot['value_node']);
        $this->assertEquals($v, $this->sot->getValueNode());
        unset($this->sot['value_node']);
        $this->assertFalse(isset($this->sot['value_node']));
        $this->sot['value_node'] = $v;
        $this->assertEquals($v, $this->sot['value_node']);
        $this->assertTrue(isset($this->sot['value_node']));
        $this->sot['value_node'] = null;
        $this->assertFalse(isset($this->sot['value_node']));
    }

    /**
     * Test invalid use of offsetSet.
     *
     * @legacy-covers ::offsetSet
     */
    public function testInvalidOffsetException(): void
    {
        $this->expectException(\RuntimeException::class);
        $this->sot[] = 'foobar';
    }
}
