<?php

/**
 * FolderResponseTest class file.
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Please update the test case below to test the model.
 */

declare(strict_types=1);

namespace NecLimDul\MarketoRest\Asset\Test\Model;

use Faker\Factory as FakerFactory;
use NecLimDul\MarketoRest\Asset\Model\ModelInterface;
use NecLimDul\MarketoRest\Asset\Model\FolderResponse;
use Neclimdul\OpenapiPhpTesting\FakeValuesTrait;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

/**
 * FolderResponseTest
 *
 * The version of the OpenAPI document: 1.0
 * Contact: 
 * Generated by: https://openapi-generator.tech
 * OpenAPI Generator version: 7.21.0
 */
#[CoversClass(FolderResponse::class)]
class FolderResponseTest extends TestCase
{
    use FakeValuesTrait;

    private FolderResponse $sot;

    /**
     * @var string[]
     */
    private array $types = [
        'access_zone_id' => 'int',
        'created_at' => '\DateTime',
        'description' => 'string',
        'folder_id' => '\NecLimDul\MarketoRest\Asset\Model\Folder',
        'folder_type' => 'string',
        'id' => 'int',
        'is_archive' => 'bool',
        'is_system' => 'bool',
        'name' => 'string',
        'parent' => '\NecLimDul\MarketoRest\Asset\Model\Folder',
        'path' => 'string',
        'updated_at' => '\DateTime',
        'url' => 'string',
        'workspace' => 'string',
    ];

    /**
     * @var scalar[][]
     */
    private array $allowedValues = [
        'folder_type' => [
            'EMAIL' => 'Email',
            'EMAIL_BATCH_PROGRAM' => 'Email Batch Program',
            'EMAIL_TEMPLATE' => 'Email Template',
            'IMAGE' => 'Image',
            'LANDING_PAGE' => 'Landing Page',
            'LANDING_PAGE_FORM' => 'Landing Page Form',
            'LANDING_PAGE_TEMPLATE' => 'Landing Page Template',
            'MARKETING_EVENT' => 'Marketing Event',
            'MARKETING_FOLDER' => 'Marketing Folder',
            'MARKETING_PROGRAM' => 'Marketing Program',
            'NURTURE_PROGRAM' => 'Nurture Program',
            'REPORT' => 'Report',
            'REVENUE_CYCLE_MODEL' => 'Revenue Cycle Model',
            'ZONE' => 'Zone',
        ],
    ];

    public function setUp(): void
    {
        parent::setUp();
        $this->setFaker(FakerFactory::create());
        $this->sot = $this->getFakeValue(FolderResponse::class, null);
    }

    /**
     * Test "FolderResponse"
     *
     * @legacy-covers ::__construct
     */
    public function testFolderResponse(): void
    {
        $this->assertInstanceOf(FolderResponse::class, $this->sot);
    }

    /**
     * @legacy-covers ::openAPITypes
     */
    public function testOpenApiTypes(): void
    {
        $this->assertEquals($this->types, FolderResponse::openAPITypes());
    }

    /**
     * @legacy-covers ::openAPIRequired
     */
    public function testOpenAPIRequired(): void
    {
        $required = $this->sot->openAPIRequired();
        $this->assertNotContains('access_zone_id', $required);
        $this->assertEquals('created_at', $required['created_at']);
        $this->assertNotContains('description', $required);
        $this->assertEquals('folder_id', $required['folder_id']);
        $this->assertEquals('folder_type', $required['folder_type']);
        $this->assertEquals('id', $required['id']);
        $this->assertEquals('is_archive', $required['is_archive']);
        $this->assertNotContains('is_system', $required);
        $this->assertEquals('name', $required['name']);
        $this->assertEquals('parent', $required['parent']);
        $this->assertEquals('path', $required['path']);
        $this->assertEquals('updated_at', $required['updated_at']);
        $this->assertEquals('url', $required['url']);
        $this->assertNotContains('workspace', $required);
    }

    /**
     * @legacy-covers ::openAPIFormats
     */
    public function testOpenAPIFormats(): void
    {
        $formats = $this->sot->openAPIFormats();
        $this->assertEquals('int32', $formats['access_zone_id']);
        $this->assertEquals('date-time', $formats['created_at']);
        $this->assertNotContains('description', $formats);
        $this->assertNotContains('folder_id', $formats);
        $this->assertNotContains('folder_type', $formats);
        $this->assertEquals('int32', $formats['id']);
        $this->assertNotContains('is_archive', $formats);
        $this->assertNotContains('is_system', $formats);
        $this->assertNotContains('name', $formats);
        $this->assertNotContains('parent', $formats);
        $this->assertNotContains('path', $formats);
        $this->assertEquals('date-time', $formats['updated_at']);
        $this->assertNotContains('url', $formats);
        $this->assertNotContains('workspace', $formats);
    }

    /**
     * @legacy-covers ::attributeMap
     */
    public function testAttributeMap(): void
    {
        $attributes = $this->sot->attributeMap();
        $this->assertEquals('accessZoneId', $attributes['access_zone_id']);
        $this->assertEquals('createdAt', $attributes['created_at']);
        $this->assertEquals('description', $attributes['description']);
        $this->assertEquals('folderId', $attributes['folder_id']);
        $this->assertEquals('folderType', $attributes['folder_type']);
        $this->assertEquals('id', $attributes['id']);
        $this->assertEquals('isArchive', $attributes['is_archive']);
        $this->assertEquals('isSystem', $attributes['is_system']);
        $this->assertEquals('name', $attributes['name']);
        $this->assertEquals('parent', $attributes['parent']);
        $this->assertEquals('path', $attributes['path']);
        $this->assertEquals('updatedAt', $attributes['updated_at']);
        $this->assertEquals('url', $attributes['url']);
        $this->assertEquals('workspace', $attributes['workspace']);
    }

    /**
     * @legacy-covers ::getters
     * @legacy-covers ::setters
     */
    public function testGettersSetters(): void
    {
        $getters = $this->sot->getters();
        $setters = $this->sot->setters();
        foreach (array_keys($this->types) as $field) {
            $this->assertTrue(isset($setters[$field]));
            $this->assertTrue(isset($getters[$field]));
        }
    }

    /**
     * @legacy-covers ::getModelName
     */
    public function testGetModelName(): void
    {
        $this->assertEquals('FolderResponse', $this->sot->getModelName());
    }

    /**
     * @legacy-covers ::setAdditionalProperties
     * @legacy-covers ::setAdditionalProperty
     * @legacy-covers ::getAdditionalProperties
     */
    public function testAdditionalProperties(): void
    {
        do {
            $property = $this->faker->word();
        } while (isset($this->sot::attributeMap()[$property]));
        $value = 'foobar';
        $this->sot[$property] = $value;
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperty($property, $value);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperties([
            $property => $value,
        ]);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        $this->assertEquals(
            $value,
            $this->sot->jsonSerialize()->$property
        );
    }

    /**
     * @legacy-covers ::setAdditionalProperty
     */
    public function testAdditionalPropertyExistingProperty(): void
    {
        $this->expectException(\AssertionError::class);
        $this->sot->setAdditionalProperty(key($this->sot::attributeMap()), 'foobar');
    }

    /**
     * @legacy-covers ::jsonSerialize
     * @legacy-covers ::__toString
     */
    public function testJson(): void
    {
        // Some minimal tests that json generates well.
        $json = json_encode($this->sot);
        $this->assertIsString($json, 'Json encoded');
        $json = json_decode($json);
        $string = json_decode((string) $this->sot);
        $this->assertEquals(
            $json,
            $string
        );
        $this->assertInstanceOf(\stdClass::class, $json);
        $this->assertInstanceOf(\stdClass::class, $string);
    }

    /**
     * Test attribute "access_zone_id"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getAccessZoneId
     * @legacy-covers ::setAccessZoneId
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyAccessZoneId(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['access_zone_id'],
            $this->allowedValues['access_zone_id'] ?? null
        );
        $this->sot->setAccessZoneId($v);
        $this->assertEquals($v, $this->sot->getAccessZoneId());
        $this->sot->setAccessZoneId(null);
        $this->assertNull($this->sot->getAccessZoneId());
        $this->sot->setAccessZoneId($v);

        $this->assertEquals($v, $this->sot['access_zone_id']);
        $v = $this->getFakeValue(
            $this->types['access_zone_id'],
            $this->allowedValues['access_zone_id'] ?? null
        );
        $this->sot['access_zone_id'] = $v;
        $this->assertTrue(isset($this->sot['access_zone_id']));
        $this->assertEquals($v, $this->sot['access_zone_id']);
        $this->assertEquals($v, $this->sot->getAccessZoneId());
        unset($this->sot['access_zone_id']);
        $this->assertFalse(isset($this->sot['access_zone_id']));
        $this->sot['access_zone_id'] = $v;
        $this->assertEquals($v, $this->sot['access_zone_id']);
        $this->assertTrue(isset($this->sot['access_zone_id']));
        $this->sot['access_zone_id'] = null;
        $this->assertFalse(isset($this->sot['access_zone_id']));
    }

    /**
     * Test attribute "created_at"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getCreatedAt
     * @legacy-covers ::setCreatedAt
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyCreatedAt(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['created_at'],
            $this->allowedValues['created_at'] ?? null
        );
        $this->sot->setCreatedAt($v);
        $this->assertEquals($v, $this->sot->getCreatedAt());

        $this->assertEquals($v, $this->sot['created_at']);
        $v = $this->getFakeValue(
            $this->types['created_at'],
            $this->allowedValues['created_at'] ?? null
        );
        $this->sot['created_at'] = $v;
        $this->assertTrue(isset($this->sot['created_at']));
        $this->assertEquals($v, $this->sot['created_at']);
        $this->assertEquals($v, $this->sot->getCreatedAt());
        unset($this->sot['created_at']);
        $this->assertFalse(isset($this->sot['created_at']));
        $this->sot['created_at'] = $v;
        $this->assertEquals($v, $this->sot['created_at']);
        $this->assertTrue(isset($this->sot['created_at']));
        $this->sot['created_at'] = null;
        $this->assertFalse(isset($this->sot['created_at']));
    }

    /**
     * Test attribute "description"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getDescription
     * @legacy-covers ::setDescription
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyDescription(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['description'],
            $this->allowedValues['description'] ?? null
        );
        $this->sot->setDescription($v);
        $this->assertEquals($v, $this->sot->getDescription());
        $this->sot->setDescription(null);
        $this->assertNull($this->sot->getDescription());
        $this->sot->setDescription($v);

        $this->assertEquals($v, $this->sot['description']);
        $v = $this->getFakeValue(
            $this->types['description'],
            $this->allowedValues['description'] ?? null
        );
        $this->sot['description'] = $v;
        $this->assertTrue(isset($this->sot['description']));
        $this->assertEquals($v, $this->sot['description']);
        $this->assertEquals($v, $this->sot->getDescription());
        unset($this->sot['description']);
        $this->assertFalse(isset($this->sot['description']));
        $this->sot['description'] = $v;
        $this->assertEquals($v, $this->sot['description']);
        $this->assertTrue(isset($this->sot['description']));
        $this->sot['description'] = null;
        $this->assertFalse(isset($this->sot['description']));
    }

    /**
     * Test attribute "folder_id"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getFolderId
     * @legacy-covers ::setFolderId
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyFolderId(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['folder_id'],
            $this->allowedValues['folder_id'] ?? null
        );
        $this->sot->setFolderId($v);
        $this->assertEquals($v, $this->sot->getFolderId());

        $this->assertEquals($v, $this->sot['folder_id']);
        $v = $this->getFakeValue(
            $this->types['folder_id'],
            $this->allowedValues['folder_id'] ?? null
        );
        $this->sot['folder_id'] = $v;
        $this->assertTrue(isset($this->sot['folder_id']));
        $this->assertEquals($v, $this->sot['folder_id']);
        $this->assertEquals($v, $this->sot->getFolderId());
        unset($this->sot['folder_id']);
        $this->assertFalse(isset($this->sot['folder_id']));
        $this->sot['folder_id'] = $v;
        $this->assertEquals($v, $this->sot['folder_id']);
        $this->assertTrue(isset($this->sot['folder_id']));
        $this->sot['folder_id'] = null;
        $this->assertFalse(isset($this->sot['folder_id']));
    }

    /**
     * Test attribute "folder_type"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getFolderType
     * @legacy-covers ::setFolderType
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyFolderType(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['folder_type'],
            $this->allowedValues['folder_type'] ?? null
        );
        $this->sot->setFolderType($v);
        $this->assertEquals($v, $this->sot->getFolderType());

        $this->assertEquals($v, $this->sot['folder_type']);
        $v = $this->getFakeValue(
            $this->types['folder_type'],
            $this->allowedValues['folder_type'] ?? null
        );
        $this->sot['folder_type'] = $v;
        $this->assertTrue(isset($this->sot['folder_type']));
        $this->assertEquals($v, $this->sot['folder_type']);
        $this->assertEquals($v, $this->sot->getFolderType());
        unset($this->sot['folder_type']);
        $this->assertFalse(isset($this->sot['folder_type']));
        $this->sot['folder_type'] = $v;
        $this->assertEquals($v, $this->sot['folder_type']);
        $this->assertTrue(isset($this->sot['folder_type']));
        $this->sot['folder_type'] = null;
        $this->assertFalse(isset($this->sot['folder_type']));
    }

    /**
     * Trivial allowed values test.
     *
     * @legacy-covers ::getFolderTypeAllowableValues()
     */
    public function testFolderTypeAllowableValues(): void
    {
        // Trivial allowed values test.
        $this->assertEquals(array_values($this->allowedValues['folder_type']), $this->sot->getFolderTypeAllowableValues());
        foreach ($this->allowedValues['folder_type'] as $v) {
            $this->sot->setFolderType($v);
            $this->assertEquals($v, $this->sot->getFolderType());
        }
    }

    /**
     * @legacy-covers ::setFolderType
     */
    public function testFolderTypeNotAllowableValues(): void
    {
        $this->markTestIncomplete('Broken my property overloading and needs additional work');
        // Find a value that's not allowed.
        do {
            $v = $this->getFakeValue($this->types['folder_type'], null);
        } while (!isset($v) || in_array($v, $this->allowedValues['folder_type'], true));
        $this->expectException(\InvalidArgumentException::class);
        $this->sot->setFolderType($v);
        throw new \Exception(var_export($v, true) . " should have triggered an error...");
    }

    /**
     * Test attribute "id"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getId
     * @legacy-covers ::setId
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyId(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['id'],
            $this->allowedValues['id'] ?? null
        );
        $this->sot->setId($v);
        $this->assertEquals($v, $this->sot->getId());

        $this->assertEquals($v, $this->sot['id']);
        $v = $this->getFakeValue(
            $this->types['id'],
            $this->allowedValues['id'] ?? null
        );
        $this->sot['id'] = $v;
        $this->assertTrue(isset($this->sot['id']));
        $this->assertEquals($v, $this->sot['id']);
        $this->assertEquals($v, $this->sot->getId());
        unset($this->sot['id']);
        $this->assertFalse(isset($this->sot['id']));
        $this->sot['id'] = $v;
        $this->assertEquals($v, $this->sot['id']);
        $this->assertTrue(isset($this->sot['id']));
        $this->sot['id'] = null;
        $this->assertFalse(isset($this->sot['id']));
    }

    /**
     * Test attribute "is_archive"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getIsArchive
     * @legacy-covers ::setIsArchive
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyIsArchive(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['is_archive'],
            $this->allowedValues['is_archive'] ?? null
        );
        $this->sot->setIsArchive($v);
        $this->assertEquals($v, $this->sot->getIsArchive());

        $this->assertEquals($v, $this->sot['is_archive']);
        $v = $this->getFakeValue(
            $this->types['is_archive'],
            $this->allowedValues['is_archive'] ?? null
        );
        $this->sot['is_archive'] = $v;
        $this->assertTrue(isset($this->sot['is_archive']));
        $this->assertEquals($v, $this->sot['is_archive']);
        $this->assertEquals($v, $this->sot->getIsArchive());
        unset($this->sot['is_archive']);
        $this->assertFalse(isset($this->sot['is_archive']));
        $this->sot['is_archive'] = $v;
        $this->assertEquals($v, $this->sot['is_archive']);
        $this->assertTrue(isset($this->sot['is_archive']));
        $this->sot['is_archive'] = null;
        $this->assertFalse(isset($this->sot['is_archive']));
    }

    /**
     * Test attribute "is_system"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getIsSystem
     * @legacy-covers ::setIsSystem
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyIsSystem(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['is_system'],
            $this->allowedValues['is_system'] ?? null
        );
        $this->sot->setIsSystem($v);
        $this->assertEquals($v, $this->sot->getIsSystem());
        $this->sot->setIsSystem(null);
        $this->assertNull($this->sot->getIsSystem());
        $this->sot->setIsSystem($v);

        $this->assertEquals($v, $this->sot['is_system']);
        $v = $this->getFakeValue(
            $this->types['is_system'],
            $this->allowedValues['is_system'] ?? null
        );
        $this->sot['is_system'] = $v;
        $this->assertTrue(isset($this->sot['is_system']));
        $this->assertEquals($v, $this->sot['is_system']);
        $this->assertEquals($v, $this->sot->getIsSystem());
        unset($this->sot['is_system']);
        $this->assertFalse(isset($this->sot['is_system']));
        $this->sot['is_system'] = $v;
        $this->assertEquals($v, $this->sot['is_system']);
        $this->assertTrue(isset($this->sot['is_system']));
        $this->sot['is_system'] = null;
        $this->assertFalse(isset($this->sot['is_system']));
    }

    /**
     * Test attribute "name"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getName
     * @legacy-covers ::setName
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyName(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['name'],
            $this->allowedValues['name'] ?? null
        );
        $this->sot->setName($v);
        $this->assertEquals($v, $this->sot->getName());

        $this->assertEquals($v, $this->sot['name']);
        $v = $this->getFakeValue(
            $this->types['name'],
            $this->allowedValues['name'] ?? null
        );
        $this->sot['name'] = $v;
        $this->assertTrue(isset($this->sot['name']));
        $this->assertEquals($v, $this->sot['name']);
        $this->assertEquals($v, $this->sot->getName());
        unset($this->sot['name']);
        $this->assertFalse(isset($this->sot['name']));
        $this->sot['name'] = $v;
        $this->assertEquals($v, $this->sot['name']);
        $this->assertTrue(isset($this->sot['name']));
        $this->sot['name'] = null;
        $this->assertFalse(isset($this->sot['name']));
    }

    /**
     * Test attribute "parent"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getParent
     * @legacy-covers ::setParent
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyParent(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['parent'],
            $this->allowedValues['parent'] ?? null
        );
        $this->sot->setParent($v);
        $this->assertEquals($v, $this->sot->getParent());

        $this->assertEquals($v, $this->sot['parent']);
        $v = $this->getFakeValue(
            $this->types['parent'],
            $this->allowedValues['parent'] ?? null
        );
        $this->sot['parent'] = $v;
        $this->assertTrue(isset($this->sot['parent']));
        $this->assertEquals($v, $this->sot['parent']);
        $this->assertEquals($v, $this->sot->getParent());
        unset($this->sot['parent']);
        $this->assertFalse(isset($this->sot['parent']));
        $this->sot['parent'] = $v;
        $this->assertEquals($v, $this->sot['parent']);
        $this->assertTrue(isset($this->sot['parent']));
        $this->sot['parent'] = null;
        $this->assertFalse(isset($this->sot['parent']));
    }

    /**
     * Test attribute "path"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getPath
     * @legacy-covers ::setPath
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyPath(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['path'],
            $this->allowedValues['path'] ?? null
        );
        $this->sot->setPath($v);
        $this->assertEquals($v, $this->sot->getPath());

        $this->assertEquals($v, $this->sot['path']);
        $v = $this->getFakeValue(
            $this->types['path'],
            $this->allowedValues['path'] ?? null
        );
        $this->sot['path'] = $v;
        $this->assertTrue(isset($this->sot['path']));
        $this->assertEquals($v, $this->sot['path']);
        $this->assertEquals($v, $this->sot->getPath());
        unset($this->sot['path']);
        $this->assertFalse(isset($this->sot['path']));
        $this->sot['path'] = $v;
        $this->assertEquals($v, $this->sot['path']);
        $this->assertTrue(isset($this->sot['path']));
        $this->sot['path'] = null;
        $this->assertFalse(isset($this->sot['path']));
    }

    /**
     * Test attribute "updated_at"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getUpdatedAt
     * @legacy-covers ::setUpdatedAt
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyUpdatedAt(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['updated_at'],
            $this->allowedValues['updated_at'] ?? null
        );
        $this->sot->setUpdatedAt($v);
        $this->assertEquals($v, $this->sot->getUpdatedAt());

        $this->assertEquals($v, $this->sot['updated_at']);
        $v = $this->getFakeValue(
            $this->types['updated_at'],
            $this->allowedValues['updated_at'] ?? null
        );
        $this->sot['updated_at'] = $v;
        $this->assertTrue(isset($this->sot['updated_at']));
        $this->assertEquals($v, $this->sot['updated_at']);
        $this->assertEquals($v, $this->sot->getUpdatedAt());
        unset($this->sot['updated_at']);
        $this->assertFalse(isset($this->sot['updated_at']));
        $this->sot['updated_at'] = $v;
        $this->assertEquals($v, $this->sot['updated_at']);
        $this->assertTrue(isset($this->sot['updated_at']));
        $this->sot['updated_at'] = null;
        $this->assertFalse(isset($this->sot['updated_at']));
    }

    /**
     * Test attribute "url"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getUrl
     * @legacy-covers ::setUrl
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyUrl(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['url'],
            $this->allowedValues['url'] ?? null
        );
        $this->sot->setUrl($v);
        $this->assertEquals($v, $this->sot->getUrl());

        $this->assertEquals($v, $this->sot['url']);
        $v = $this->getFakeValue(
            $this->types['url'],
            $this->allowedValues['url'] ?? null
        );
        $this->sot['url'] = $v;
        $this->assertTrue(isset($this->sot['url']));
        $this->assertEquals($v, $this->sot['url']);
        $this->assertEquals($v, $this->sot->getUrl());
        unset($this->sot['url']);
        $this->assertFalse(isset($this->sot['url']));
        $this->sot['url'] = $v;
        $this->assertEquals($v, $this->sot['url']);
        $this->assertTrue(isset($this->sot['url']));
        $this->sot['url'] = null;
        $this->assertFalse(isset($this->sot['url']));
    }

    /**
     * Test attribute "workspace"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getWorkspace
     * @legacy-covers ::setWorkspace
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyWorkspace(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['workspace'],
            $this->allowedValues['workspace'] ?? null
        );
        $this->sot->setWorkspace($v);
        $this->assertEquals($v, $this->sot->getWorkspace());
        $this->sot->setWorkspace(null);
        $this->assertNull($this->sot->getWorkspace());
        $this->sot->setWorkspace($v);

        $this->assertEquals($v, $this->sot['workspace']);
        $v = $this->getFakeValue(
            $this->types['workspace'],
            $this->allowedValues['workspace'] ?? null
        );
        $this->sot['workspace'] = $v;
        $this->assertTrue(isset($this->sot['workspace']));
        $this->assertEquals($v, $this->sot['workspace']);
        $this->assertEquals($v, $this->sot->getWorkspace());
        unset($this->sot['workspace']);
        $this->assertFalse(isset($this->sot['workspace']));
        $this->sot['workspace'] = $v;
        $this->assertEquals($v, $this->sot['workspace']);
        $this->assertTrue(isset($this->sot['workspace']));
        $this->sot['workspace'] = null;
        $this->assertFalse(isset($this->sot['workspace']));
    }

    /**
     * Test invalid use of offsetSet.
     *
     * @legacy-covers ::offsetSet
     */
    public function testInvalidOffsetException(): void
    {
        $this->expectException(\RuntimeException::class);
        $this->sot[] = 'foobar';
    }
}
