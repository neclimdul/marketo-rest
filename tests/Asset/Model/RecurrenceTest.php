<?php

/**
 * RecurrenceTest class file.
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Please update the test case below to test the model.
 */

declare(strict_types=1);

namespace NecLimDul\MarketoRest\Asset\Test\Model;

use Faker\Factory as FakerFactory;
use NecLimDul\MarketoRest\Asset\Model\ModelInterface;
use NecLimDul\MarketoRest\Asset\Model\Recurrence;
use Neclimdul\OpenapiPhpTesting\FakeValuesTrait;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

/**
 * RecurrenceTest
 *
 * The version of the OpenAPI document: 1.0
 * Contact: 
 * Generated by: https://openapi-generator.tech
 * OpenAPI Generator version: 7.21.0
 */
#[CoversClass(Recurrence::class)]
class RecurrenceTest extends TestCase
{
    use FakeValuesTrait;

    private Recurrence $sot;

    /**
     * @var string[]
     */
    private array $types = [
        'start_at' => '\DateTime',
        'end_at' => '\DateTime',
        'interval_type' => 'string',
        'interval' => 'int',
        'weekday_only' => 'bool',
        'weekday_mask' => 'string[]',
        'day_of_month' => 'int',
        'day_of_week' => 'string',
        'week_of_month' => 'int',
    ];

    /**
     * @var scalar[][]
     */
    private array $allowedValues = [
        'interval_type' => [
            'DAILY' => 'Daily',
            'WEEKLY' => 'Weekly',
            'MONTHLY' => 'Monthly',
        ],
        'day_of_week' => [
            'MONDAY' => 'Monday',
            'TUESDAY' => 'Tuesday',
            'WEDNESDAY' => 'Wednesday',
            'THURSDAY' => 'Thursday',
            'FRIDAY' => 'Friday',
            'SATURDAY' => 'Saturday',
            'SUNDAY' => 'Sunday',
        ],
    ];

    public function setUp(): void
    {
        parent::setUp();
        $this->setFaker(FakerFactory::create());
        $this->sot = $this->getFakeValue(Recurrence::class, null);
    }

    /**
     * Test "Recurrence"
     *
     * @legacy-covers ::__construct
     */
    public function testRecurrence(): void
    {
        $this->assertInstanceOf(Recurrence::class, $this->sot);
    }

    /**
     * @legacy-covers ::openAPITypes
     */
    public function testOpenApiTypes(): void
    {
        $this->assertEquals($this->types, Recurrence::openAPITypes());
    }

    /**
     * @legacy-covers ::openAPIRequired
     */
    public function testOpenAPIRequired(): void
    {
        $required = $this->sot->openAPIRequired();
        $this->assertEquals('start_at', $required['start_at']);
        $this->assertEquals('end_at', $required['end_at']);
        $this->assertEquals('interval_type', $required['interval_type']);
        $this->assertEquals('interval', $required['interval']);
        $this->assertEquals('weekday_only', $required['weekday_only']);
        $this->assertEquals('weekday_mask', $required['weekday_mask']);
        $this->assertEquals('day_of_month', $required['day_of_month']);
        $this->assertEquals('day_of_week', $required['day_of_week']);
        $this->assertEquals('week_of_month', $required['week_of_month']);
    }

    /**
     * @legacy-covers ::openAPIFormats
     */
    public function testOpenAPIFormats(): void
    {
        $formats = $this->sot->openAPIFormats();
        $this->assertEquals('date-time', $formats['start_at']);
        $this->assertEquals('date-time', $formats['end_at']);
        $this->assertNotContains('interval_type', $formats);
        $this->assertEquals('int32', $formats['interval']);
        $this->assertNotContains('weekday_only', $formats);
        $this->assertNotContains('weekday_mask', $formats);
        $this->assertEquals('int32', $formats['day_of_month']);
        $this->assertEquals('int32', $formats['day_of_week']);
        $this->assertEquals('int32', $formats['week_of_month']);
    }

    /**
     * @legacy-covers ::attributeMap
     */
    public function testAttributeMap(): void
    {
        $attributes = $this->sot->attributeMap();
        $this->assertEquals('startAt', $attributes['start_at']);
        $this->assertEquals('endAt', $attributes['end_at']);
        $this->assertEquals('intervalType', $attributes['interval_type']);
        $this->assertEquals('interval', $attributes['interval']);
        $this->assertEquals('weekdayOnly', $attributes['weekday_only']);
        $this->assertEquals('weekdayMask', $attributes['weekday_mask']);
        $this->assertEquals('dayOfMonth', $attributes['day_of_month']);
        $this->assertEquals('dayOfWeek', $attributes['day_of_week']);
        $this->assertEquals('weekOfMonth', $attributes['week_of_month']);
    }

    /**
     * @legacy-covers ::getters
     * @legacy-covers ::setters
     */
    public function testGettersSetters(): void
    {
        $getters = $this->sot->getters();
        $setters = $this->sot->setters();
        foreach (array_keys($this->types) as $field) {
            $this->assertTrue(isset($setters[$field]));
            $this->assertTrue(isset($getters[$field]));
        }
    }

    /**
     * @legacy-covers ::getModelName
     */
    public function testGetModelName(): void
    {
        $this->assertEquals('Recurrence', $this->sot->getModelName());
    }

    /**
     * @legacy-covers ::setAdditionalProperties
     * @legacy-covers ::setAdditionalProperty
     * @legacy-covers ::getAdditionalProperties
     */
    public function testAdditionalProperties(): void
    {
        do {
            $property = $this->faker->word();
        } while (isset($this->sot::attributeMap()[$property]));
        $value = 'foobar';
        $this->sot[$property] = $value;
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperty($property, $value);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        unset($this->sot[$property]);
        $this->assertArrayNotHasKey($property, $this->sot);
        $this->sot->setAdditionalProperties([
            $property => $value,
        ]);
        $this->assertArrayHasKey($property, $this->sot);
        $this->assertEquals($value, $this->sot->getAdditionalProperties()[$property]);
        $this->assertEquals(
            $value,
            $this->sot->jsonSerialize()->$property
        );
    }

    /**
     * @legacy-covers ::setAdditionalProperty
     */
    public function testAdditionalPropertyExistingProperty(): void
    {
        $this->expectException(\AssertionError::class);
        $this->sot->setAdditionalProperty(key($this->sot::attributeMap()), 'foobar');
    }

    /**
     * @legacy-covers ::jsonSerialize
     * @legacy-covers ::__toString
     */
    public function testJson(): void
    {
        // Some minimal tests that json generates well.
        $json = json_encode($this->sot);
        $this->assertIsString($json, 'Json encoded');
        $json = json_decode($json);
        $string = json_decode((string) $this->sot);
        $this->assertEquals(
            $json,
            $string
        );
        $this->assertInstanceOf(\stdClass::class, $json);
        $this->assertInstanceOf(\stdClass::class, $string);
    }

    /**
     * Test attribute "start_at"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getStartAt
     * @legacy-covers ::setStartAt
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyStartAt(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['start_at'],
            $this->allowedValues['start_at'] ?? null
        );
        $this->sot->setStartAt($v);
        $this->assertEquals($v, $this->sot->getStartAt());

        $this->assertEquals($v, $this->sot['start_at']);
        $v = $this->getFakeValue(
            $this->types['start_at'],
            $this->allowedValues['start_at'] ?? null
        );
        $this->sot['start_at'] = $v;
        $this->assertTrue(isset($this->sot['start_at']));
        $this->assertEquals($v, $this->sot['start_at']);
        $this->assertEquals($v, $this->sot->getStartAt());
        unset($this->sot['start_at']);
        $this->assertFalse(isset($this->sot['start_at']));
        $this->sot['start_at'] = $v;
        $this->assertEquals($v, $this->sot['start_at']);
        $this->assertTrue(isset($this->sot['start_at']));
        $this->sot['start_at'] = null;
        $this->assertFalse(isset($this->sot['start_at']));
    }

    /**
     * Test attribute "end_at"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getEndAt
     * @legacy-covers ::setEndAt
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyEndAt(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['end_at'],
            $this->allowedValues['end_at'] ?? null
        );
        $this->sot->setEndAt($v);
        $this->assertEquals($v, $this->sot->getEndAt());

        $this->assertEquals($v, $this->sot['end_at']);
        $v = $this->getFakeValue(
            $this->types['end_at'],
            $this->allowedValues['end_at'] ?? null
        );
        $this->sot['end_at'] = $v;
        $this->assertTrue(isset($this->sot['end_at']));
        $this->assertEquals($v, $this->sot['end_at']);
        $this->assertEquals($v, $this->sot->getEndAt());
        unset($this->sot['end_at']);
        $this->assertFalse(isset($this->sot['end_at']));
        $this->sot['end_at'] = $v;
        $this->assertEquals($v, $this->sot['end_at']);
        $this->assertTrue(isset($this->sot['end_at']));
        $this->sot['end_at'] = null;
        $this->assertFalse(isset($this->sot['end_at']));
    }

    /**
     * Test attribute "interval_type"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getIntervalType
     * @legacy-covers ::setIntervalType
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyIntervalType(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['interval_type'],
            $this->allowedValues['interval_type'] ?? null
        );
        $this->sot->setIntervalType($v);
        $this->assertEquals($v, $this->sot->getIntervalType());

        $this->assertEquals($v, $this->sot['interval_type']);
        $v = $this->getFakeValue(
            $this->types['interval_type'],
            $this->allowedValues['interval_type'] ?? null
        );
        $this->sot['interval_type'] = $v;
        $this->assertTrue(isset($this->sot['interval_type']));
        $this->assertEquals($v, $this->sot['interval_type']);
        $this->assertEquals($v, $this->sot->getIntervalType());
        unset($this->sot['interval_type']);
        $this->assertFalse(isset($this->sot['interval_type']));
        $this->sot['interval_type'] = $v;
        $this->assertEquals($v, $this->sot['interval_type']);
        $this->assertTrue(isset($this->sot['interval_type']));
        $this->sot['interval_type'] = null;
        $this->assertFalse(isset($this->sot['interval_type']));
    }

    /**
     * Trivial allowed values test.
     *
     * @legacy-covers ::getIntervalTypeAllowableValues()
     */
    public function testIntervalTypeAllowableValues(): void
    {
        // Trivial allowed values test.
        $this->assertEquals(array_values($this->allowedValues['interval_type']), $this->sot->getIntervalTypeAllowableValues());
        foreach ($this->allowedValues['interval_type'] as $v) {
            $this->sot->setIntervalType($v);
            $this->assertEquals($v, $this->sot->getIntervalType());
        }
    }

    /**
     * @legacy-covers ::setIntervalType
     */
    public function testIntervalTypeNotAllowableValues(): void
    {
        $this->markTestIncomplete('Broken my property overloading and needs additional work');
        // Find a value that's not allowed.
        do {
            $v = $this->getFakeValue($this->types['interval_type'], null);
        } while (!isset($v) || in_array($v, $this->allowedValues['interval_type'], true));
        $this->expectException(\InvalidArgumentException::class);
        $this->sot->setIntervalType($v);
        throw new \Exception(var_export($v, true) . " should have triggered an error...");
    }

    /**
     * Test attribute "interval"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getInterval
     * @legacy-covers ::setInterval
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyInterval(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['interval'],
            $this->allowedValues['interval'] ?? null
        );
        $this->sot->setInterval($v);
        $this->assertEquals($v, $this->sot->getInterval());

        $this->assertEquals($v, $this->sot['interval']);
        $v = $this->getFakeValue(
            $this->types['interval'],
            $this->allowedValues['interval'] ?? null
        );
        $this->sot['interval'] = $v;
        $this->assertTrue(isset($this->sot['interval']));
        $this->assertEquals($v, $this->sot['interval']);
        $this->assertEquals($v, $this->sot->getInterval());
        unset($this->sot['interval']);
        $this->assertFalse(isset($this->sot['interval']));
        $this->sot['interval'] = $v;
        $this->assertEquals($v, $this->sot['interval']);
        $this->assertTrue(isset($this->sot['interval']));
        $this->sot['interval'] = null;
        $this->assertFalse(isset($this->sot['interval']));
    }

    /**
     * Test attribute "weekday_only"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getWeekdayOnly
     * @legacy-covers ::setWeekdayOnly
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyWeekdayOnly(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['weekday_only'],
            $this->allowedValues['weekday_only'] ?? null
        );
        $this->sot->setWeekdayOnly($v);
        $this->assertEquals($v, $this->sot->getWeekdayOnly());

        $this->assertEquals($v, $this->sot['weekday_only']);
        $v = $this->getFakeValue(
            $this->types['weekday_only'],
            $this->allowedValues['weekday_only'] ?? null
        );
        $this->sot['weekday_only'] = $v;
        $this->assertTrue(isset($this->sot['weekday_only']));
        $this->assertEquals($v, $this->sot['weekday_only']);
        $this->assertEquals($v, $this->sot->getWeekdayOnly());
        unset($this->sot['weekday_only']);
        $this->assertFalse(isset($this->sot['weekday_only']));
        $this->sot['weekday_only'] = $v;
        $this->assertEquals($v, $this->sot['weekday_only']);
        $this->assertTrue(isset($this->sot['weekday_only']));
        $this->sot['weekday_only'] = null;
        $this->assertFalse(isset($this->sot['weekday_only']));
    }

    /**
     * Test attribute "weekday_mask"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getWeekdayMask
     * @legacy-covers ::setWeekdayMask
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyWeekdayMask(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['weekday_mask'],
            $this->allowedValues['weekday_mask'] ?? null
        );
        $this->sot->setWeekdayMask($v);
        $this->assertEquals($v, $this->sot->getWeekdayMask());

        $this->assertEquals($v, $this->sot['weekday_mask']);
        $v = $this->getFakeValue(
            $this->types['weekday_mask'],
            $this->allowedValues['weekday_mask'] ?? null
        );
        $this->sot['weekday_mask'] = $v;
        $this->assertTrue(isset($this->sot['weekday_mask']));
        $this->assertEquals($v, $this->sot['weekday_mask']);
        $this->assertEquals($v, $this->sot->getWeekdayMask());
        unset($this->sot['weekday_mask']);
        $this->assertFalse(isset($this->sot['weekday_mask']));
        $this->sot['weekday_mask'] = $v;
        $this->assertEquals($v, $this->sot['weekday_mask']);
        $this->assertTrue(isset($this->sot['weekday_mask']));
        $this->sot['weekday_mask'] = null;
        $this->assertFalse(isset($this->sot['weekday_mask']));
    }

    /**
     * Test attribute "day_of_month"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getDayOfMonth
     * @legacy-covers ::setDayOfMonth
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyDayOfMonth(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['day_of_month'],
            $this->allowedValues['day_of_month'] ?? null
        );
        $this->sot->setDayOfMonth($v);
        $this->assertEquals($v, $this->sot->getDayOfMonth());

        $this->assertEquals($v, $this->sot['day_of_month']);
        $v = $this->getFakeValue(
            $this->types['day_of_month'],
            $this->allowedValues['day_of_month'] ?? null
        );
        $this->sot['day_of_month'] = $v;
        $this->assertTrue(isset($this->sot['day_of_month']));
        $this->assertEquals($v, $this->sot['day_of_month']);
        $this->assertEquals($v, $this->sot->getDayOfMonth());
        unset($this->sot['day_of_month']);
        $this->assertFalse(isset($this->sot['day_of_month']));
        $this->sot['day_of_month'] = $v;
        $this->assertEquals($v, $this->sot['day_of_month']);
        $this->assertTrue(isset($this->sot['day_of_month']));
        $this->sot['day_of_month'] = null;
        $this->assertFalse(isset($this->sot['day_of_month']));
    }

    /**
     * Test attribute "day_of_week"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getDayOfWeek
     * @legacy-covers ::setDayOfWeek
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyDayOfWeek(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['day_of_week'],
            $this->allowedValues['day_of_week'] ?? null
        );
        $this->sot->setDayOfWeek($v);
        $this->assertEquals($v, $this->sot->getDayOfWeek());

        $this->assertEquals($v, $this->sot['day_of_week']);
        $v = $this->getFakeValue(
            $this->types['day_of_week'],
            $this->allowedValues['day_of_week'] ?? null
        );
        $this->sot['day_of_week'] = $v;
        $this->assertTrue(isset($this->sot['day_of_week']));
        $this->assertEquals($v, $this->sot['day_of_week']);
        $this->assertEquals($v, $this->sot->getDayOfWeek());
        unset($this->sot['day_of_week']);
        $this->assertFalse(isset($this->sot['day_of_week']));
        $this->sot['day_of_week'] = $v;
        $this->assertEquals($v, $this->sot['day_of_week']);
        $this->assertTrue(isset($this->sot['day_of_week']));
        $this->sot['day_of_week'] = null;
        $this->assertFalse(isset($this->sot['day_of_week']));
    }

    /**
     * Trivial allowed values test.
     *
     * @legacy-covers ::getDayOfWeekAllowableValues()
     */
    public function testDayOfWeekAllowableValues(): void
    {
        // Trivial allowed values test.
        $this->assertEquals(array_values($this->allowedValues['day_of_week']), $this->sot->getDayOfWeekAllowableValues());
        foreach ($this->allowedValues['day_of_week'] as $v) {
            $this->sot->setDayOfWeek($v);
            $this->assertEquals($v, $this->sot->getDayOfWeek());
        }
    }

    /**
     * @legacy-covers ::setDayOfWeek
     */
    public function testDayOfWeekNotAllowableValues(): void
    {
        $this->markTestIncomplete('Broken my property overloading and needs additional work');
        // Find a value that's not allowed.
        do {
            $v = $this->getFakeValue($this->types['day_of_week'], null);
        } while (!isset($v) || in_array($v, $this->allowedValues['day_of_week'], true));
        $this->expectException(\InvalidArgumentException::class);
        $this->sot->setDayOfWeek($v);
        throw new \Exception(var_export($v, true) . " should have triggered an error...");
    }

    /**
     * Test attribute "week_of_month"
     *
     * @legacy-covers ::__construct
     * @legacy-covers ::getWeekOfMonth
     * @legacy-covers ::setWeekOfMonth
     * @legacy-covers ::offsetExists
     * @legacy-covers ::offsetGet
     * @legacy-covers ::offsetSet
     * @legacy-covers ::offsetUnset
     */
    public function testPropertyWeekOfMonth(): void
    {
        // @todo can we assert anything useful about the default?
        $v = $this->getFakeValue(
            $this->types['week_of_month'],
            $this->allowedValues['week_of_month'] ?? null
        );
        $this->sot->setWeekOfMonth($v);
        $this->assertEquals($v, $this->sot->getWeekOfMonth());

        $this->assertEquals($v, $this->sot['week_of_month']);
        $v = $this->getFakeValue(
            $this->types['week_of_month'],
            $this->allowedValues['week_of_month'] ?? null
        );
        $this->sot['week_of_month'] = $v;
        $this->assertTrue(isset($this->sot['week_of_month']));
        $this->assertEquals($v, $this->sot['week_of_month']);
        $this->assertEquals($v, $this->sot->getWeekOfMonth());
        unset($this->sot['week_of_month']);
        $this->assertFalse(isset($this->sot['week_of_month']));
        $this->sot['week_of_month'] = $v;
        $this->assertEquals($v, $this->sot['week_of_month']);
        $this->assertTrue(isset($this->sot['week_of_month']));
        $this->sot['week_of_month'] = null;
        $this->assertFalse(isset($this->sot['week_of_month']));
    }

    /**
     * Test invalid use of offsetSet.
     *
     * @legacy-covers ::offsetSet
     */
    public function testInvalidOffsetException(): void
    {
        $this->expectException(\RuntimeException::class);
        $this->sot[] = 'foobar';
    }
}
